...
type Iterator struct {
	Iter *C.hyperleveldb_iterator_t
}

// Valid returns false only when an Iterator has iterated past either the
// first or the last key in the database.
func (it *Iterator) Valid() bool {
	return ucharToBool(C.hyperleveldb_iter_valid(it.Iter))
}
...
func (it *Iterator) Key() []byte {
	var klen C.size_t
	kdata := C.hyperleveldb_iter_key(it.Iter, &klen)
	if kdata == nil {
		return nil
	}
...
func (it *Iterator) Value() []byte {
	var vlen C.size_t
	vdata := C.hyperleveldb_iter_value(it.Iter, &vlen)
	if vdata == nil {
		return nil
	}
...
func (it *Iterator) Next() {
	C.hyperleveldb_iter_next(it.Iter)
}
...
func (it *Iterator) Prev() {
	C.hyperleveldb_iter_prev(it.Iter)
}
...
func (it *Iterator) SeekToFirst() {
	C.hyperleveldb_iter_seek_to_first(it.Iter)
}
...
func (it *Iterator) SeekToLast() {
	C.hyperleveldb_iter_seek_to_last(it.Iter)
}
...
func (it *Iterator) Seek(key []byte) {
	C.hyperleveldb_iter_seek(it.Iter, (*C.char)(unsafe.Pointer(&key[0])), C.size_t(len(key)))
}
...
func (it *Iterator) GetError() error {
	var errStr *C.char
	C.hyperleveldb_iter_get_error(it.Iter, &errStr)
	if errStr != nil {
...
func (it *Iterator) Close() {
	C.hyperleveldb_iter_destroy(it.Iter)
	it.Iter = nil
}
